...
namespace Basic {
BasicPipeline::BasicPipeline(BasicDevice &device,
                             const std::string &vertFilePath,
                             const std::string &fragFilePath,
                             const PipelineConfigInfo &configInfo)
    : basicDevice{device} {
  createBasicPipeline(vertFilePath, fragFilePath, configInfo);
}

BasicPipeline::~BasicPipeline() {
  vkDestroyShaderModule(basicDevice.device(), vertShaderModule, nullptr);
  vkDestroyShaderModule(basicDevice.device(), fragShaderModule, nullptr);
...
void BasicPipeline::createBasicPipeline(const std::string &vertFilePath,
  ...
  assert(configInfo.pipelineLayout != VK_NULL_HANDLE &&
         "You need to provide pipelineLayout in configInfo");
  assert(configInfo.renderPass != VK_NULL_HANDLE &&
         "You need to provide renderPass in configInfo");
  ...
  if (vkCreateGraphicsPipelines(basicDevice.device(), VK_NULL_HANDLE, 1,
...
void BasicPipeline::createShaderModule(const std::vector<char> &code,
                                       VkShaderModule *shaderModule) {
  VkShaderModuleCreateInfo createInfo{};
  createInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
  createInfo.codeSize = code.size();
  createInfo.pCode = reinterpret_cast<const uint32_t *>(code.data());
  if (vkCreateShaderModule(basicDevice.device(), &createInfo, nullptr,
                           shaderModule) != VK_SUCCESS) {
    throw std::runtime_error("failed to create shader module");
  }
}
PipelineConfigInfo BasicPipeline::defaultPipelineConfigInfo(uint32_t width,
                                                            uint32_t height) {
